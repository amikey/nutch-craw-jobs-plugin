...
public class HtmlParser implements Parser {
  ...
  //added in order to crawl more agressivly
  private boolean relaxedCrawlRestrictions = false;
  ...
  public ParseResult getParse(Content content) {
    ...
    if (relaxedCrawlRestrictions || !metaTags.getNoIndex()) {               // okay to index
    ...
    if (relaxedCrawlRestrictions|| !metaTags.getNoFollow()) {              // okay to follow links
    ...
    //if present, cookies are passed to metadata
      if(content.getMetadata().get("Cookie")!=null){
        	...
        	for (int i = 0; i < outlinks.length; i++) {
  			...
  			outlink.getMetadata().put(new Text("PARSE_PARENT_URL"), new Text(content.getBaseUrl()));
    ...
    ParseResult filteredParse = this.htmlParseFilters.filter(content, parseResult, 
                                                             metaTags, root);
    if (!relaxedCrawlRestrictions && metaTags.getNoCache()) {             // not okay to cache
  ...
  public void setConf(Configuration conf) {
    ...
    this.relaxedCrawlRestrictions = getConf().getBoolean("relax.crawl.restrictions", false);
  }
